...
class TestProjectRoute(unittest.TestCase):
    ...
    def test_get_project_rating(self):
        ...
        with app.container.sqlite_repo.override(
            ...
            response = self.client.get(
                self.prefix + f"/project/{project_id}/avg_rating"
            )

            # Assert
            self.assertEqual(response.status_code, 200)
            self.assertEqual(
                response.json(), {"id": project_id, "rating": expected_rating}
            )
            self.mock_sqlite_repo.get_project_avg_rating.assert_called_once_with(
                project_id
            )
    ...
    def test_get_project_rating_wrong_id(self):
        ...
        with app.container.sqlite_repo.override(
            ...
            self.assertEqual(response.status_code, 404)
            self.assertEqual(
                response.json(), {"detail": {"id": 2, "Error": "Project not found"}}
            )
